...
export const RecordMedia: React.FC<{ media?: RecordContentBlock[] }> = ({ media = [] }) => {
  ...
  return (
    ...
    {media.filter((item) => item.type !== 'image').map((item, index) => {
      ...
      if (item.type === 'link') return <a key={'link' + index} href={item.url} target="_blank" rel="noreferrer" className="flex max-w-[460px] overflow-hidden rounded-md border border-slate-200/80 bg-slate-50 transition-colors hover:border-sakura-300 dark:border-slate-700 dark:bg-slate-900/60 dark:hover:border-sakura-600">{item.image && <img src={item.image} alt="" className="h-20 w-20 shrink-0 object-cover sm:h-24 sm:w-24" />}<span className="min-w-0 p-2.5 sm:p-3"><span className="flex items-center gap-1 text-xs text-sakura-600"><ExternalLink className="h-3 w-3" /> 外部链接</span><strong className="mt-0.5 block truncate text-sm text-slate-800 dark:text-slate-100">{item.title || item.url}</strong>{item.description && <span className="mt-0.5 block line-clamp-2 text-xs text-slate-500">{item.description}</span>}</span></a>;
      if (item.type === 'music') return <a key={'music' + index} href={item.url} target="_blank" rel="noreferrer" className="flex max-w-[460px] items-center gap-2.5 rounded-md border border-slate-200/80 p-2.5 transition-colors hover:border-sakura-300 dark:border-slate-700 dark:hover:border-sakura-600"><div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-md bg-sakura-100 dark:bg-sakura-950/50">{item.cover ? <img src={item.cover} alt="" className="h-full w-full object-cover" /> : <Music2 className="h-4 w-4 text-sakura-600" />}</div><span className="min-w-0"><strong className="block truncate text-sm dark:text-slate-100">{item.title}</strong><span className="text-xs text-slate-500">{item.artist || '音乐'}</span></span><Play className="ml-auto h-4 w-4 text-sakura-600" /></a>;
      const Icon = item.type === 'douban-book' ? BookOpen : Film;
      return <a key={item.type + index} href={item.url} target="_blank" rel="noreferrer" className="flex max-w-[460px] items-center gap-2.5 rounded-md border border-slate-200/80 p-2.5 transition-colors hover:border-sakura-300 dark:border-slate-700 dark:hover:border-sakura-600">{item.cover ? <img src={item.cover} alt="" className="h-16 w-12 rounded-md object-cover" /> : <Icon className="h-4 w-4 text-sakura-600" />}<span><span className="text-[10px] text-sakura-600">{item.type === 'douban-book' ? '豆瓣读书' : '豆瓣电影'}</span><strong className="block text-sm dark:text-slate-100">{item.title}</strong><span className="text-xs text-slate-500">{item.description}</span></span></a>;
    })}
